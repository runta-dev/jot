...
test('identical browser text is referenced once without changing stored history',async()=>{
 ...
 const evaluate:Evaluate=async request=>{
  ...
  assert.ok(request.questions.action.criteria.draft_message);
...
test('draft_message passes Jev wording as the hints argument',async()=>{
 const draft={name:'draft_message',description:'Draft',parameters:{type:'object' as const,required:['hints'],additionalProperties:false as const,properties:{hints:{type:'string',description:'Jev short answer.'}}},async *execute(){return {status:'ok' as const,text:'ok'};}};
 let nexts=0;
 const evaluate:Evaluate=async request=>{
  if(request.questions.action)return {model:'test',answers:{action:{type:'choice',choice:'draft_message',confidence:1,probabilities:{draft_message:1}}}};
