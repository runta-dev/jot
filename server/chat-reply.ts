...
export async function* generateChatReply(key:string,messages:ChatMessage[],signal:AbortSignal,options:{draft?:DraftAnswer;evaluate?:Evaluate;tools?:ToolFactory[];extraTools?:ToolFactory[];maxTurns?:number;maxInputTokens?:number}={}):AsyncGenerator<AgentEvent>{
 ...
 const tools=options.tools??[...createAgentTools({key,evaluate,draft:options.draft,remainingInput:()=>maxInput-usage.inputTokens}),...(options.extraTools??[])];
