...
export async function* generateChatReply(key:string,messages:ChatMessage[],signal:AbortSignal,options:{evaluate?:Evaluate;tools?:ToolFactory[];maxTurns?:number;maxInputTokens?:number}={}):AsyncGenerator<AgentEvent>{
 ...
 const tools=options.tools??createAgentTools({key,evaluate,remainingInput:()=>maxInput-usage.inputTokens});
