...
const dir = `experiments/results/stopping-${new Date().toISOString().replace(/[:.]/g, "-")}`;
  ...
  JSON.stringify(
    {
      ...
      lexiconHash: hash,
      lexiconSize: 4096,
      protocol: "research/R2.2-stopping-protocol.md",
      prompts,
    },
    null,
    2,
...
const jobs = prompts.flatMap((prompt) =>
  ["word", "word"].map((mode, repeat) => ({ prompt, mode, repeat })),
);
let cursor = 0;
const rows: any[] = [];
...
await Promise.all(
  Array.from({ length: 2 }, async () => {
    while (cursor < jobs.length) {
      const { prompt, mode, repeat } = jobs[cursor++];
            ...
            next:
              mode === "char"
                ...
                : {
                    type: "choice",
                    instructions:
                      "Which group contains the best NEXT WHOLE WORD or punctuation mark to append to assistant_reply_so_far while directly answering the user? Each group lists its words. Choose the next grammatical word, not merely a word related to the topic. Continue the existing reply without restarting or repeating it.",
                    criteria,
                  },
            complete: {
              type: "noul",
            ...
            const wordRequest = {
              ...
              questions: {
                next: {
                  type: "choice",
                  instructions:
                    "Choose the next whole word or punctuation mark to continue assistant_reply_so_far as a direct, concise English answer to the user. Normal spaces and sentence capitalization are inserted by the application. Do not restart the answer.",
