import { quantityKindBank } from "./lib/qudt.ts";
import {
  readFile,
  mkdir,
  writeFile,
...
cases.push(
  { id: "cork", question: "Why does cork float on water?", group: "factual" },
  { id: "helium", question: "Why does helium rise in air?", group: "factual" },
  {
    id: "ethanol",
    question:
      "Compare ethanol and water in boiling point at atmospheric pressure.",
    group: "factual",
  },
  {
    id: "unknown_alloys",
    question:
      "Is the density of material QX-73419 greater than that of material RY-90831?",
    group: "unknown",
  },
);
const qudt = await quantityKindBank();
const qNames = new Set(
  qudt.flatMap((p) => p.aliases.map((x: string) => x.toLowerCase())),
);
...
for (const line of (
  ...
  const at = line.indexOf("|"),
    f = line.slice(0, at).trim().split(/\s+/),
    count = parseInt(f[3], 16);
  let cursor = 4 + 2 * count;
  const pointers = Number(f[cursor++]);
  nodes.set(f[0], {
    id: "w" + f[0],
    aliases: Array.from({ length: count }, (_, i) =>
      f[4 + 2 * i].replaceAll("_", " "),
  ...
  for (let i = 0; i < pointers; i++) {
    const [symbol, target, pos] = f.slice(cursor + 4 * i, cursor + 4 * i + 4);
    if ((symbol === "@" || symbol === "@i") && pos === "n")
      children.set(target, [...(children.get(target) || []), f[0]]);
  }
}
const seen = new Set(["04923519"]),
  queue = ["04923519"];
while (queue.length) {
...
bank.splice(
  ...
  ...[...seen]
    .map((id) => nodes.get(id))
    .filter(
      (p) => p && !p.aliases.some((a: string) => qNames.has(a.toLowerCase())),
    ),
);
const dir = `experiments/results/quantity-index-${new Date().toISOString().replace(/[:.]/g, "-")}`;
await mkdir(dir, { recursive: true });
await copyFile("node_modules/wordnet-db/LICENSE", `${dir}/WORDNET-LICENSE.txt`);
await copyFile(".cache/qudt/LICENSE.md", `${dir}/QUDT-LICENSE.md`);
await writeFile(
  `${dir}/manifest.json`,
  JSON.stringify(
    {
      protocol: "research/R13.1-quantity-index-protocol.md",
      cases,
      propertyBank: bank,
    },
    null,
...
for (const c of cases) {
  ...
  try {
    ...
    const explicit = bank.filter((p) =>
      p.aliases.some((alias: string) => {
        const name = alias.toLowerCase();
        if (generic.has(name)) return false;
        const q = c.question.toLowerCase();
        let at = q.indexOf(name);
        while (at >= 0) {
          const before = q.slice(0, at),
            after = q.slice(at + name.length);
          if (
            (!before || !/[a-z]$/.test(before)) &&
            (!after || !/^[a-z]/.test(after)) &&
            (/(?:\bin|\bregarding|\bhigher|\blower|\bgreater|\bless|\bsame)\s+$/.test(
              before,
            ) ||
              /^\s+of\b/.test(after))
          )
            return true;
          at = q.indexOf(name, at + 1);
        }
        return false;
      }),
    );
    const searchBank = explicit.length ? explicit : bank;
    const finalists = new Map<string, any>();
    const groups: any[][] = [];
    for (let i = 0; i < searchBank.length; i += 64)
      groups.push(searchBank.slice(i, i + 64));
    searchInfo = {
      mode: explicit.length ? "explicit" : "catalog",
      candidates: searchBank.length,
