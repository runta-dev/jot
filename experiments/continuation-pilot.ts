...
const dir=`experiments/results/continuation-${new Date().toISOString().replaceAll(':','-')}`;
await mkdir(dir,{recursive:true});
await writeFile(`${dir}/manifest.json`,JSON.stringify({hash,cases,protocol:'research/R15.1-continuation-protocol.md'},null,2));
...
for(const c of cases){
 ...
 for(const user of c.turns){
  ...
  const row={id:c.id,user,text:prefix,reason,requests,inputTokens,outputTokens,ms:Date.now()-started,firstMs};results.push(row);console.log(JSON.stringify(row));
