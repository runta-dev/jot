...
const format='full';
const arm=process.argv[2];if(!['greedy','branch'].includes(arm))throw Error('Expected greedy or branch');
...
const dir=`experiments/results/bounded-branch-${arm}-${new Date().toISOString().replaceAll(':','-')}`;
await mkdir(dir,{recursive:true});
await writeFile(`${dir}/source.ts`,await readFile('experiments/bounded-branch.ts','utf8'));
await writeFile(`${dir}/manifest.json`,JSON.stringify({hash,cases,arm,protocol:'research/R16-bounded-branching.md'},null,2));
