...
cases.push(
  {
    id: "fresh_missing",
    draft: "Good mornng.",
    instruction: "Correct spelling only; preserve the greeting.",
  ...
  {
    id: "fresh_duplicate",
    draft: "It is is working.",
    instruction: "Remove accidental repeated words; preserve meaning.",
    context: "It is working now.",
  ...
  },
  {
    ...
    draft: "We was late.",
    instruction:
      "Fix grammar while preserving first-person plural and past tense.",
    context: "The speakers were late.",
    expected: "We were late.",
    split: "fresh",
  },
  {
    id: "fresh_spaces",
    draft: "That is  fine.",
    instruction:
      "Remove accidental repeated spaces in prose; change nothing else.",
    context: "Ordinary prose.",
    expected: "That is fine.",
    split: "fresh",
  },
  {
    id: "fresh_question",
    draft: "Where is it.",
    instruction: "Correct terminal punctuation for a direct question.",
    context: "A question asking where something is.",
    expected: "Where is it?",
    split: "fresh",
  },
  {
    id: "fresh_correct",
    draft: "Do not change this.",
    instruction: "Correct mistakes only. Keep correct text unchanged.",
    context: "A grammatical instruction.",
    expected: "Do not change this.",
    split: "fresh",
  },
  {
    id: "fresh_literal",
    draft: 'print("x  y")',
    instruction:
      "Do not change valid Python or string literals. Preserve correct code verbatim.",
    context: "The code must print x, two spaces, and y.",
    expected: 'print("x  y")',
    split: "fresh",
  },
  {
    id: "fresh_amount",
    draft: "The fee is $2,040.",
    instruction: "Correct mistakes only; preserve valid number formatting.",
    context: "The fee is exactly two thousand forty dollars.",
    expected: "The fee is $2,040.",
    split: "fresh",
  },
);
...
const dir = `experiments/results/action-recall-${new Date().toISOString().replace(/[:.]/g, "-")}`;
await mkdir(dir, { recursive: true });
await writeFile(
  `${dir}/manifest.json`,
  JSON.stringify(
    {
      protocol: "research/R3.2-action-recall-protocol.md",
      cases,
      alphabet,
      functionWords,
    },
    ...
    while (cursor < cases.length) {
      ...
      try {
        ...
        const retainedActions = Object.entries(
          ...
          .filter(([key]) => Object.hasOwn(actions, key))
          .sort((a, b) => b[1] - a[1])
          .slice(0, 2)
          .map(([key]) => key);
        ...
        for (const action of retainedActions) {
          if (action === "normalize_spaces")
            ...
            const ordered = Object.entries(
              first.data.answers[action].probabilities as Record<
                string,
                number
              >,
            )
            ...
            for (const [id] of ordered) {
              const position = locations[action].find(
                (x) => x.key === id,
              )!.index;
