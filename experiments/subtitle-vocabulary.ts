...
const dir=`experiments/results/subtitle-vocabulary-${new Date().toISOString().replaceAll(':','-')}`;
await mkdir(dir,{recursive:true});
await writeFile(`${dir}/source.ts`,await readFile('experiments/subtitle-vocabulary.ts','utf8'));
await writeFile(`${dir}/manifest.json`,JSON.stringify({hash,cases,arm,protocol:'research/R20-vocabulary-protocol.md'},null,2));
