...
if(!['baseline','operation'].includes(arm))throw Error('Expected baseline or operation');
const format='full';
...
const cases=JSON.parse(await readFile('research/evaluations/general-chat-v1.json','utf8')).filter((c:any)=>['explanation','memory','correction'].includes(c.id));
const dir=`experiments/results/operation-${arm}-${new Date().toISOString().replaceAll(':','-')}`;
await mkdir(dir,{recursive:true});
await writeFile(`${dir}/manifest.json`,JSON.stringify({hash,cases,arm,protocol:'research/R15.4-operation-protocol.md'},null,2));
...
for(const c of cases){
 ...
 for(const user of c.turns){
  history.push({role:'user',content:user});let operation:unknown=undefined;let prefix='',requests=0,inputTokens=0,outputTokens=0,reason='limit',firstMs:number|null=null;const started=Date.now();
  async function ask(stage:string,questions:Record<string,unknown>){
   ...
   const request={model:'jev-latest',state:{conversation:history,assistant:{name:'Jev',provider:'TypeSafe'},reply_so_far:prefix,...(operation?{requested_operation:operation}:{})},questions};
  ...
  try{
   if(arm==='operation'){
    const criteria={answer:'Answer the latest question using conversation context.',clarify:'Ask for information needed to resolve ambiguity.',shorten:'Make the preceding answer shorter while retaining its central meaning.',simplify:'Explain the preceding answer in easier language; do not add unnecessary complexity.',correct:'Apply the latest correction and use the updated facts rather than stale information.',continue:'Continue the preceding content coherently.',translate:'Translate the requested content into the requested language.',other:'Follow the latest request using conversation context; none of the other descriptions fits.'};
    const selected=await ask('operation',{operation:{type:'choice',instructions:'Which operation is the latest user asking the assistant to perform? Use prior messages to resolve references. Select the request, not a response or an answer.',criteria}});
    const key=selected.operation.choice as keyof typeof criteria;
    if(!Object.hasOwn(criteria,key))throw Error('Invalid operation');
    operation={kind:key,instruction:criteria[key],confidence:selected.operation.confidence};
   }
   ...
   for(let step=0;step<40;step++){
    const proposed=await ask('propose',Object.fromEntries(groups(active).map((bank,i)=>[`g${i}`,{type:'choice',instructions:(arm==='operation'?'Follow requested_operation.instruction. ':'')+'Choose the next word to append to reply_so_far to answer the latest user message coherently in English. Continue unfinished sentences. Choose OTHER if no listed word fits.',criteria:{...Object.fromEntries(bank.map(w=>[w,null])),OTHER:'No listed word fits'}}])));
  ...
  const row={arm,operation,id:c.id,user,text:prefix,reason,requests,inputTokens,outputTokens,ms:Date.now()-started,firstMs};results.push(row);console.log(JSON.stringify(row));
