...
const cases=JSON.parse(await readFile('research/evaluations/general-chat-v1.json','utf8')).filter((c:any)=>['chinese','creative','memory'].includes(c.id));
const dir=`experiments/results/morphology-conversation-${new Date().toISOString().replaceAll(':','-')}`;
await mkdir(dir,{recursive:true});
await writeFile(`${dir}/manifest.json`,JSON.stringify({hash,cases,format,protocol:'research/R15.7-morphology-conversation.md'},null,2));
