...
const cases=JSON.parse(await readFile('research/evaluations/general-chat-v1.json','utf8')).filter((c:any)=>['advice','unknown','chinese','creative'].includes(c.id));
const dir=`experiments/results/broad-diagnostic-${new Date().toISOString().replaceAll(':','-')}`;
await mkdir(dir,{recursive:true});
await writeFile(`${dir}/manifest.json`,JSON.stringify({hash,cases,format,protocol:'research/R15.5-broad-diagnostic.md'},null,2));
