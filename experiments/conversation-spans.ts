...
const cases=[{id:'retain',turns:['Rewrite politely: Send the invoice by Monday.','Make it shorter, keep the deadline.']},{id:'replace',turns:['Write a warm one-sentence invitation to lunch.','Make it formal instead, still one sentence.']}];const dir=`experiments/results/conversation-spans-${new Date().toISOString().replaceAll(':','-')}`;await mkdir(dir,{recursive:true});await writeFile(`${dir}/manifest.json`,JSON.stringify({protocol:'research/R37-conversation-spans.md',cases},null,2));
...
for(const arm of ['baseline','copy'])for(const c of cases){const history:Message[]=[];for(const user of c.turns){history.push({role:'user',content:user});
 ...
 for(const message of history)for(const sentence of message.content.split(/[.!?\n]+/)){
