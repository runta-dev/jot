...
const dir = `experiments/results/lexical-${new Date().toISOString().replace(/[:.]/g, "-")}`;
await mkdir(dir, { recursive: true });
await writeFile(
  `${dir}/manifest.json`,
  JSON.stringify(
    {
      protocol: "research/R7-lexical-knowledge-protocol.md",
      cases,
      vocabulary,
    },
    null,
...
await Promise.all(
  Array.from({ length: 2 }, async () => {
    while (cursor < jobs.length) {
      ...
      try {
        const first = await run("group", {
          group: {
            type: "choice",
            instructions:
              "Which word group contains the best concept for semantic_role in a useful, accurate answer to user_question? Select based on meaning, not a word merely appearing in the question.",
            criteria: Object.fromEntries(
              Object.entries(groups).map(([id, list]) => [id, list.join(", ")]),
            ),
          },
        });
        const chosen = Object.entries(
          first.answers.group.probabilities as Record<string, number>,
        )
          .filter(([id]) => groups[id])
          .sort((a, b) => b[1] - a[1])
          .slice(0, 2)
          .map(([id]) => id);
        shortlist = chosen.flatMap((id) => groups[id]);
