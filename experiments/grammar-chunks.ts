...
const format='full';
const arm='greedy';
const corpus=await readFile('.cache/research/frequencywords/reference-en.txt','utf8');
const hash=createHash('sha256').update(corpus).digest('hex');
if(hash!=='5351ff405b1126ef555791dd4d9798a48e3e9a501a9fc481a9da957752cfb458')throw Error('Corpus changed');
const base=corpus.trim().split(/\r?\n/).map(line=>line.split(/\s+/)[0]).filter(w=>/^[a-z][a-z']*$/.test(w)).slice(0,4096);
...
const dir=`experiments/results/grammar-chunks-${new Date().toISOString().replaceAll(':','-')}`;
await mkdir(dir,{recursive:true});
await writeFile(`${dir}/source.ts`,await readFile('experiments/grammar-chunks.ts','utf8'));
await writeFile(`${dir}/manifest.json`,JSON.stringify({hash,cases,arm,protocol:'research/R22-grammar-chunks.md'},null,2));
...
function chunks(words:string[]){
 const out:string[]=[];
 for(const word of words){const lower=word.toLowerCase();
  const aux=lower==='i'?['am','was','have','had','can','will']:['you','we','they'].includes(lower)?['are','were','have','had','can','will']:['he','she','it'].includes(lower)?['is','was','has','had','can','will']:[];
  for(const tail of aux)out.push(`${word==='i'?'I':word} ${tail}`);
  if(['of','in','on','at','to','from','with','for'].includes(lower))for(const tail of ['the','a'])out.push(`${word} ${tail}`);
  if(['and','but'].includes(lower))for(const tail of ['it','they'])out.push(`${word} ${tail}`);
 }return out;
}
const results:any[]=[];
 ...
 for(const user of c.turns){
  ...
  try{
   ...
   async function proposeAt(at:string){
    ...
    const expanded=[...new Set([...originals,...originals.flatMap(inflections),...chunks(originals)])];
   ...
   for(let step=0;step<40;){
    ...
    }else{const next=decision.criteria[decision.answer.choice];const used=next.slice(prefix.length).trim().split(/\s+/).length;if(step+used>40)break;await appendFile(`${dir}/commits.jsonl`,JSON.stringify({user,prefix,next,used})+'\n');prefix=next;step+=used;firstMs??=Date.now()-started;}
   }
