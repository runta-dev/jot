...
const dir=`experiments/results/promotion-word-${new Date().toISOString().replaceAll(':','-')}`;
await mkdir(dir,{recursive:true});
await writeFile(`${dir}/source.ts`,await readFile('experiments/promotion-word.ts','utf8'));
await writeFile(`${dir}/manifest.json`,JSON.stringify({hash,cases,arm,protocol:'research/R27-prototype-promotion.md'},null,2));
