...
const prompts=['Why do wet clothes dry?','A child dropped a glass. What might happen next?','How can I begin a difficult task?','Why does a metal spoon get warm in hot soup?','A person forgot an umbrella in the rain. What might happen?','How can I organize scattered notes?'];
const dir=`experiments/results/lexicalized-tree-${new Date().toISOString().replaceAll(':','-')}`;await mkdir(dir,{recursive:true});await writeFile(`${dir}/manifest.json`,JSON.stringify({protocol:'research/R26.1-compact-replay.md',prompts},null,2));
await writeFile(`${dir}/source.ts`,await readFile('experiments/lexicalized-tree.ts','utf8'));
const rows=[];
for(const prompt of prompts){let tree:Tree=hole('clause'),requests=0,inputTokens=0,reason='limit',text='';const started=Date.now(),chosenSenses:any[]=[];
 async function ask(stage:string,questions:any){if(inputTokens>=500000)throw Error('Input budget');const request={model:'jev-latest',state:{user_request:prompt,partial_tree:tree,selected_senses:chosenSenses},questions};await appendFile(`${dir}/requests.jsonl`,JSON.stringify({prompt,stage,request})+'\n');let response;try{response=await researchCall(request);}catch(error){await appendFile(`${dir}/errors.jsonl`,JSON.stringify({prompt,stage,error:String(error)})+'\n');throw error;}requests+=response.attempts;inputTokens+=response.data.usage?.input_tokens??0;await appendFile(`${dir}/trace.jsonl`,JSON.stringify({prompt,stage,request,response})+'\n');return response.data.answers;}
 try{
 for(let step=0;step<30;step++){
  ...
  if(target.sort==='np'){
   const pool=[...new Set([...words,...(prompt.toLowerCase().match(/[a-z]+/g)??[])])].filter(w=>lex[w]?.N);
   const banks=Array.from({length:Math.ceil(pool.length/180)},(_,i)=>pool.slice(i*180,(i+1)*180));
   const answers=await ask('np-heads',Object.fromEntries(banks.map((bank,i)=>[`g${i}`,{type:'choice',instructions:`Which noun belongs at hole ${JSON.stringify(target.path)} in partial_tree so it answers user_request? Respect already bound roles.`,criteria:Object.fromEntries(bank.map(w=>[w,null]))}])));
   const nouns=[...new Set(Object.values(answers).flatMap((a:any)=>Object.entries(a.probabilities).sort((a:any,b:any)=>b[1]-a[1]).slice(0,2).map(([w])=>w)))];
   const retained=actions.filter(a=>a.replacement.kind!=='np');
   const concrete:Action[]=nouns.flatMap(value=>structures('np').flatMap(a=>{
    if(a.replacement.kind!=='np')return [];
    const node=a.replacement;
    if(lex[value].N.cnt==='no'&&(node.plural||node.determiner==='a'))return [];
    const replacement:Tree={...node,noun:{kind:'word',sort:'noun',value}};
    return [{label:`${node.determiner} ${node.plural?'plural':'singular'} ${value}`,replacement}];
   }));actions=[...retained,...concrete];if(actions.length>254)throw Error('NP options exceed API budget');
  }
  actions=budgetActions(tree,target.path,actions);
  if(!actions.length){reason='unsupported';break;}
  const answer=await ask('expand',{next:{type:'choice',instructions:`Expand hole ${JSON.stringify(target.path)} so the complete tree will answer user_request. Options contain only the replacement subtree for this hole; the complete tree remains in state.partial_tree. Every choice must respect existing bound roles and selected_senses. Choose UNSUPPORTED if no option permits an adequate response; do not force a false claim.`,criteria:{...Object.fromEntries(actions.map((a,i)=>[`a${i}`,{description:a.label,replacement_subtree:a.replacement}])),UNSUPPORTED:'Available structures or lexical candidates cannot express the needed answer.'}}});
