...
type Options={instructions?:string;toolResults?:unknown;evaluate?:Evaluate;vocabulary?:string[];lexicon?:'english'|'prompt';forms?:(word:string)=>string[];maxSteps?:number;maxInputTokens?:number};
...
export async function* generateWordReply(key:string,messages:Message[],signal:AbortSignal,options:Options={}):AsyncGenerator<WordEvent>{
 ...
 const promptWords=conversationLexemes(messages);
 const base=options.vocabulary??(options.lexicon==='prompt'?promptWords:vocabulary),forms=options.forms??inflections,promptOnly=options.lexicon==='prompt'||!!options.vocabulary;
 ...
 try{
  const context=promptOnly?promptWords:conversationLexemes(messages);
  const extra=promptOnly?[]:['Jot','Jev','TypeSafe'];
  const banks=groups([...new Set([...base,...context,...extra])]);
  ...
  const active=[...new Set([...(promptOnly?base:base.slice(0,400)),...context,...extra,...Object.values(selected).flatMap(a=>a.type==='choice'?top(a,32):[])])];
  for(let i=0;i<maxSteps;i++){
   ...
   const originals=[...new Set(Object.values(proposed).flatMap(a=>a.type==='choice'?top(a,4):[]))];
   ...
   if(next.type!=='choice')throw Error('TypeSafe returned a non-choice continuation.');
