...
export function BrowserPanel({open,onClose,children}:{open:boolean;onClose:()=>void;children?:ReactNode}){
 const panel=useRef<HTMLElement>(null);
 const manuallySized=useRef(false);
 const [available,setAvailable]=useState(960);
 const maximum=()=>Math.max(240,available-240);
 ...
 useLayoutEffect(()=>{
  ...
  const measure=()=>{
   ...
   setAvailable(space);
   setWidth(w=>manuallySized.current?Math.max(240,Math.min(space-240,w)):space/2);
  };
  measure();
 ...
 return <aside ref={panel} id="browser-panel" className={`browser-panel ${open?"is-open":""} ${resizing?"is-resizing":""}`} aria-label="Browser" aria-hidden={!open} inert={!open} style={{width:open?width:0,"--browser-panel-width":`${width}px`} as CSSProperties}>
  ...
  <header className="browser-panel-header"><button className="icon-button browser-collapse-button" aria-label="Collapse browser" title="Collapse browser" onClick={onClose}><ChevronsRight size={18}/></button><span>Browser</span></header>
