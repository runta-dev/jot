...
const icons:Record<string,typeof SquareTerminal>={calculate:SquareTerminal,read_context:FileSearch,draft_message:PenLine,compose_reply:PenLine};
 ...
 return <section className="tool-activity" aria-label="Tool calls">
  ...
  <div className="tool-list">
   {calls.map(call=>{
    ...
    const drafting=call.name==='draft_message'||call.name==='compose_reply';
    const command=invocation(call),Icon=call.name.startsWith('browser_')?BrowserToolIcon:icons[call.name]??Wrench;
    const label=drafting?'Draft message':`${verb} ${command}`;
    ...
    return <details className={`tool-row ${state.toLowerCase()}`} key={`${call.id}-${working?"active":"settled"}`}>
     <summary aria-label={`${label} — ${state}${timing?` — ${timing}`:''}`} title={label}>
      ...
      <span className="tool-summary">{running?<span className="tool-shimmer">{drafting?'Draft message':<><span className="tool-action">{verb}</span>{' '}{command}</>}</span>:drafting?'Draft message':<><span className="tool-action">{verb}</span>{' '}{command}</>}</span>
