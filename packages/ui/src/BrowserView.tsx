...
export function BrowserView({chatId,active,onTakeOver}:{chatId:string;active:boolean;onTakeOver:()=>void}){
 ...
 useEffect(()=>{
  if(!active)return;
  const controller=new AbortController();lifetime.current=controller;inputQueue.current=[];sending.current=false;clearTimeout(targetTimer.current);setAgentTarget(undefined);latestStatus.current=idle;pendingAddress.current=null;setStatus(idle);setHasFrame(false);frame.current=undefined;setAddress('');setError('');
  const renderer=browserFrames(image.current!,painted=>{frame.current=painted;setHasFrame(true);});
  const source=new EventSource(`${base}/events`);
  source.onmessage=e=>{const event=JSON.parse(e.data) as BrowserEvent;if(event.type==='frame')renderer.push(event.frame);else{if(event.status.target){clearTimeout(targetTimer.current);setAgentTarget(event.status.target);}else if(event.status.loading||event.status.state==='closed'||event.status.state==='error'){clearTimeout(targetTimer.current);setAgentTarget(undefined);}else{clearTimeout(targetTimer.current);targetTimer.current=setTimeout(()=>setAgentTarget(undefined),450);}
latestStatus.current=event.status;setStatus(event.status);if(!editingAddress.current&&!pendingAddress.current)setAddress(event.status.url==='about:blank'?'':event.status.url);if(event.status.error)setError(event.status.error);else if(event.status.state==='ready'&&!event.status.loading)setError('');}};
  source.onerror=()=>setError('Browser connection interrupted. Reconnecting…');source.onopen=()=>setError('');
  return()=>{clearTimeout(targetTimer.current);renderer.close();source.close();controller.abort();navigation.current?.abort();};
 },[base,active]);
 useEffect(()=>{
 ...
 function input(body:BrowserInput){clearTimeout(targetTimer.current);setAgentTarget(undefined);
 ...
 return <div className="browser-view">
  ...
  <div className="browser-screen" ref={screen}>
   <canvas ref={image} hidden={!hasFrame} role="img" aria-label={status.title?`Browser: ${status.title}`:'Live browser page'}
    ...
    />
   {hasFrame&&agentTarget&&<div className="browser-agent-overlay" aria-label={`Jot: ${agentTarget.action} ${agentTarget.name}`} style={{aspectRatio:`${agentTarget.viewport.width} / ${agentTarget.viewport.height}`}}>
    <div className="browser-agent-target" style={{left:`${agentTarget.rect.x/agentTarget.viewport.width*100}%`,top:`${agentTarget.rect.y/agentTarget.viewport.height*100}%`,width:`${agentTarget.rect.width/agentTarget.viewport.width*100}%`,height:`${agentTarget.rect.height/agentTarget.viewport.height*100}%`}} />
