...
export const READ_SNAPSHOT=String.raw`(() => {
 ...
 cache.pageKey=()=>JSON.stringify([performance.timeOrigin,location.href,...roots.flatMap(root=>[...root.querySelectorAll('input,textarea,select')]).filter(e=>!['password','file','hidden'].includes(e.type)).map(e=>[identify(e),e.value,e.checked??null,e.selectedIndex??null])]);
 cache.guard=e=>{if(!e?.isConnected||!visible(e))return null;return JSON.stringify([role(e),name(e),e.value??'',e.checked??null,e.getAttribute('href'),e.getAttribute('aria-expanded'),e.disabled??false,e.readOnly??false,(e.closest('form,dialog,article,li,tr')||e.parentElement)?.innerText?.slice(0,1200)||'']);};
 const elements=[];
 for(const root of roots)for(const e of root.querySelectorAll('a[href],button,input,textarea,select,summary,[contenteditable="true"],[role="button"],[role="link"],[role="option"],[role="tab"],[role="checkbox"],[role="radio"],[role="combobox"],[role="textbox"],[role="searchbox"],[role="menuitem"]')){
  ...
  const r=e.getBoundingClientRect(),x=r.x+r.width/2,y=r.y+r.height/2;if(x<0||y<0||x>=innerWidth||y>=innerHeight)continue;
 ...
 return {interruption:verificationFrame&&verificationText?'verification':undefined,pageKey:cache.pageKey(),url:location.href,title:document.title,headings:roots.flatMap(root=>[...root.querySelectorAll('h1,h2,h3,[role=heading]')]).filter(visible).map(e=>e.innerText.trim()).filter(Boolean).slice(0,80),text:texts.join('\n').slice(0,8000),elements:elements.slice(0,200),scroll:{x:scrollX,y:scrollY,height:document.documentElement.scrollHeight,viewportHeight:innerHeight},viewport:{width:innerWidth,height:innerHeight}};
})()`;
